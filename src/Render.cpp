...
namespace hvk
{
	namespace render
	{
		...
		HRESULT CreateDevice(ComPtr<IDXGIFactory4> factory, ComPtr<IDXGIAdapter1> hardwareAdapter, ComPtr<ID3D12Device5>& deviceOut)
		{
			...
			hr = D3D12CreateDevice(hardwareAdapter.Get(), D3D_FEATURE_LEVEL_12_1, IID_PPV_ARGS(&deviceOut));
			return hr;
		}
		...
		HRESULT CheckRaytracingSupport(ComPtr<ID3D12Device5> device, bool& support)
		{
			...
			support = SUCCEEDED(hr) && caps.RaytracingTier >= D3D12_RAYTRACING_TIER_1_0;
			return hr;
		}
		...
		D3D12_INDEX_BUFFER_VIEW CreateIndexBuffer(ComPtr<ID3D12Device> device, const uint8_t* ibData, size_t length, IndexSize indexSize, ComPtr<ID3D12Resource>& ibOut)
		{
			...
			const auto size = length * static_cast<size_t>(indexSize);
			...
			uint8_t* ibBegin;
			ibOut->Map(0, &readRange, reinterpret_cast<void**>(&ibBegin));
			memcpy(ibBegin, ibData, size);
			ibOut->Unmap(0, nullptr);

			D3D12_INDEX_BUFFER_VIEW ibView;
			ibView.BufferLocation = ibOut->GetGPUVirtualAddress();
			ibView.Format = indexSize == IndexSize::Index16 ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT;
		...
		HRESULT CompileAndAddShaderToLibrary(IDxcLibrary* lib, IDxcCompiler* compiler, IDxcIncludeHandler* includeHandler, LPCWSTR filename, LPCWSTR entrypoint, LPCWSTR profile, IDxcBlob** outBlob)
		{
			...
			HRESULT hr = E_FAIL;
			hr = lib->CreateBlobFromFile(filename, &codePage, &pShaderText);
			if (SUCCEEDED(hr))
			{
				hr = compiler->Compile(pShaderText, filename, entrypoint, profile, nullptr, 0, nullptr, 0, includeHandler, &result);
				if (SUCCEEDED(hr))
				{
					result->GetStatus(&hr);
					if (SUCCEEDED(hr))
					...
					else
					{
						...
						HRESULT errorHr = result->GetErrorBuffer(&errors);
						if (SUCCEEDED(errorHr) && errors)
						{
							//wprintf(L"Shader compilation failed:\n%s\n", static_cast<const char*>(errors->GetBufferPointer()));
